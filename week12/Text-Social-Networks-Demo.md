...
`unnest_tokens`, and then we also load the `ggwordcloud` library for
obvious reasons.
...
```
    ...
    ## # A tibble: 915 × 12
    ##    ID     WTFCTR LANGUAGE  P862_1_T P862_2_T P862_3_T P863_1_T P863_2_T P863_3_T
    ##    <chr>   <dbl> <chr+lbl> <chr>    <chr>    <chr>    <chr>    <chr>    <chr>   
    ##  1 017558  0.32  1 [Engli… "I WON'… ""       ""       "THE DE… ""       ""      
    ##  2 017560  1.41  1 [Engli… "THAT S… ""       ""       ""       ""       ""      
    ##  3 017562  1.23  1 [Engli… ""       ""       ""       ""       ""       ""      
    ##  4 017565  0.668 1 [Engli… ""       ""       ""       ""       ""       ""      
    ##  5 017539  0.23  1 [Engli… "did a … "do alo… ""       "huband… ""       ""      
    ##  6 017567  0.23  1 [Engli… "UMM I … "I THIN… ""       "I GUES… ""       ""      
    ##  7 017544  0.728 1 [Engli… "DONT K… ""       ""       ""       ""       ""      
    ##  8 017575  0.263 1 [Engli… "A CHAN… ""       ""       "HER PA… ""       ""      
    ##  9 017554  0.348 1 [Engli… "I LIKE… "SHE'S … "MAYBE … "SHE NE… ""       ""      
    ## 10 017579  0.267 1 [Engli… "SHE HA… "AND TH… ""       "THAT S… ""       ""      
    ## # ℹ 905 more rows
    ## # ℹ 3 more variables: D16 <dbl>, ideo_x <chr>, clinton_oe <chr>
...
for that counting, and sort accordingly. Next we use `slice` to give us
the top 20 words.

Then we call ggplot and the geom_text_wordcloud function. We can format
it like any other geom call in ggplot (here setting the color to dark
green). We then call scale_size_area which controls how word frequencies
are mapped to the size of words in the word cloud. Specifically, it
ensures that the area of each word is proportional to its frequency (n),
rather than having the word’s height or width be directly proportional
to frequency, creating a more visually accurate representation of the
data. The max_size = 20 argument sets the maximum font size that will be
used for the most frequent word. The least frequent words in your top 20
will be smaller, with their sizes scaled proportionally based on their
frequencies.

Without scale_size_area(), ggplot would use its default size scaling,
which might not create as intuitive a visual relationship between
frequency and word size. This function makes sure that if one word
appears twice as often as another, it will have roughly twice the area
(not twice the height), which is generally easier for viewers to
interpret correctly.
...
d4 %>% 
   ...
   slice(1:20) %>% #change count as you see fit
   ggplot(aes(label = word, size = n)) +
   geom_text_wordcloud(color = "darkgreen") +
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->
...
Next we group by the ideology variable and take only the top 20 words
for each ideology group. We still call geom_text_wordcloud, but then add
in a facet_wrap based on `ideo_x`, then set our colors, set the
scale_size_area to a smaller value (since there are three word clouds).
...
d4 %>% 
   ...
   group_by(ideo_x) %>%
   slice(1:20) %>% #change count as you see fit
   ggplot(aes(label = word, size = n, color = ideo_x)) +
   geom_text_wordcloud() +
   facet_wrap(~ ideo_x) +
   scale_color_manual(values = c("Lib" = "darkblue", "Mod" = "grey", "Cons" = "darkred")) +
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-7-1.png)<!-- -->
...
d3 %>% 
   ...
   anti_join(custom_stop_words) %>%
   count(word, wt = WTFCTR, sort = T) %>%
   slice(1:30) %>% #change count as you see fit
   ggplot(aes(label = word, size = n)) +
   geom_text_wordcloud(color = "black") +
   scale_size_area(max_size = 15) +
   theme_minimal()
```
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->
...
d3 %>% 
   ...
   count(ideo_x, word, wt = WTFCTR, sort = T) %>%
   group_by(ideo_x) %>%
   slice(1:15) %>% #change count as you see fit
   ggplot(aes(label = word, size = n, color = ideo_x)) +
   geom_text_wordcloud() +
   facet_wrap(~ ideo_x) +
   scale_color_manual(values = c("Lib" = "darkblue", "Mod" = "grey", "Cons" = "darkred")) +
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-17-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-18-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-19-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-20-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-21-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-22-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-23-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-24-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-25-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-26-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-28-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-29-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-30-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-31-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-32-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-34-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-36-1.png)<!-- -->
...
![](Text-Social-Networks-Demo_files/figure-gfm/unnamed-chunk-38-1.png)<!-- -->
