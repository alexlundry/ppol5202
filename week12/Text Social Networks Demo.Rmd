...
Now we are ready to create our first word cloud. Here we do just a single cloud. First, we have to load the `tidytext` library which allows us to easily extract words from the open ended text using `unnest_tokens`, and then we also load the `ggwordcloud` library for obvious reasons.
...
Then we call ggplot and the geom_text_wordcloud function. We can format it like any other geom call in ggplot (here setting the color to dark green). We then call scale_size_area which controls how word frequencies are mapped to the size of words in the word cloud. Specifically, it ensures that the area of each word is proportional to its frequency (n), rather than having the word's height or width be directly proportional to frequency, creating a more visually accurate representation of the data. The max_size = 20 argument sets the maximum font size that will be used for the most frequent word. The least frequent words in your top 20 will be smaller, with their sizes scaled proportionally based on their frequencies.

Without scale_size_area(), ggplot would use its default size scaling, which might not create as intuitive a visual relationship between frequency and word size. This function makes sure that if one word appears twice as often as another, it will have roughly twice the area (not twice the height), which is generally easier for viewers to interpret correctly.
...
Next we'll do a comparison wordcloud which allows us to create a faceted wordcloud based on some categorical characteristic. In this case, we'll do it on Ideology. We have the same prep work, but this time the `count` function includes the variable we want to do a comparison on - `ideo_x`. Next we group by the ideology variable and take only the top 20 words for each ideology group. We still call geom_text_wordcloud, but then add in a facet_wrap based on `ideo_x`, then set our colors, set the scale_size_area to a smaller value (since there are three word clouds).
...
And finally you can facet by both a node and edge characteristic, using `facet_graph`:
